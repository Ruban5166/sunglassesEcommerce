...
const order = async (req, res) => { 
    try {
      ...
      const totalProducts = await ordercollection.countDocuments({ availability: true });
...
//         console.log(email)
//         let wallet = await usercollection.find({email:email})
//         console.log('this is order cancel',wallet)
//         const id = req.params.id;  
//         const order = await ordercollection.find({_id:id})
//         const totalAmount = order.reduce((acc, order) => acc + parseInt(order.payment.amount), 0);
//         console.log(wallet.walletbalance)
//         wallet.walletbalance += totalAmount
//         console.log('this is after deduction wallet balance',wallet.walletbalance)
...
//     }
// }
// this is the order cancel amount push to wallet history
const orderCancel = async (req, res) => {
    try {
        let email = req.session.user;
        console.log(email);
        
        // Assuming usercollection is the model for user documents
        let wallet = await usercollection.findOne({ email: email });
        console.log('this is order cancel', wallet);
        ...
        let wallettotal = Number(totalAmount)
        if (isNaN(wallettotal)) {
            throw new Error('Invalid totalAmount');
        }
        console.log(wallet.walletbalance); 
        wallet.walletbalance += wallettotal;
        console.log('this is after deduction wallet balance', wallet.walletbalance);
        
        // Push refund details to wallethistory array
